...
//Datos de WiFi
//const char* ssid = "Redmi A1";  // Aquí debes poner el nombre de tu red
//const char* password = "12345678";  // Aquí debes poner la contraseña de tu red
//const char* ssid = "IZZI-EF0D";  // Aquí debes poner el nombre de tu red
//const char* password = "C863FC34EF0D";  // Aquí debes poner la contraseña de tu red

const char* ssid = "INFINITUM9285_2.4";  // Aquí debes poner el nombre de tu red
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  for (int i = 0; i <= length-1; i++) {  // Se imprime y concatena el mensaje
...
if (String(topic) == "Ejemplo/Mensaje/LCD") { 
    // myservo1.write (SERVO_OPENED);
    /*lcd.setCursor(1,0); 
    lcd.print("Mensaje de telegram"); 
    Serial.println("Mensaje de telegram");
    lcd.setCursor(0,1); 
...
void EnvioMensaje()
{
  Serial.println(json); // Se imprime en monitor solo para poder visualizar que el string esta correctamente creado
    ...
    client.publish("Ejemplo/Mensaje/LCD", char_array); 
 }
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("Ejemplo/Mensaje/LCD"); // Esta función realiza la suscripción al tema
